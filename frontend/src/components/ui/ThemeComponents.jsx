...
export const StatsCard = ({ 
  ...
  return (
    <div 
      className={`${colors.bg} p-4 rounded-lg border ${colors.border} ${cursorClass} ${className}`}
      onClick={onClick}
    >
      ...
      {onClick && (
        <div className={`text-xs mt-2 ${colors.text} opacity-75`}>
    ...
    <div className="fixed inset-0 z-50 overflow-y-auto">
      ...
      <div className="flex min-h-full items-center justify-center p-4">
        <div className={`relative w-full ${sizes[size]} ${className}`}>
          <div className={`${colorClasses.bg.primary} rounded-lg shadow-xl`}>
            {/* Header */}
            {title && (
              <div className={`flex items-center justify-between p-6 border-b ${colorClasses.border.primary}`}>
                <h3 className={`text-lg font-semibold ${colorClasses.text.primary}`}>
                  {title}
                </h3>
                <button
                  onClick={onClose}
                  className={`${colorClasses.text.tertiary} hover:${colorClasses.text.secondary} transition-colors`}
                >
